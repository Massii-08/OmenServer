...
class BoerseScraper:
    ...
    async def start(self):
        ...
        # Cherche le Chromium système si celui de Playwright n'est pas installé
        launch_args = {
            "headless": self.headless,
            "args": ["--no-sandbox", "--disable-gpu", "--disable-dev-shm-usage"],
        }
        system_chromium = shutil.which("chromium-browser") or shutil.which("chromium") or shutil.which("google-chrome")
        if system_chromium:
            launch_args["executable_path"] = system_chromium
            logger.info(f"🌐 Utilisation du Chromium système: {system_chromium}")
        
        try:
            self._browser = await self._playwright.chromium.launch(**launch_args)
        except Exception as e:
            # Si le launch sans executable_path échoue, forcer le Chromium système
