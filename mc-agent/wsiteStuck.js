'use strict';
// Abandon d'un chantier de minage (`wsite`) menant à une IMPASSE SÈCHE → oubli du wsite + relocate (live 27/07, world_mn9).
// Miroir SEC de `wsiteDrown` : vécu NethBot4 en boucle 15× consécutives `descend_via_home_wsite → drop_ahead`,
// 0 minerai, vivant-en-panne. Un échec de descente SEC (drop_ahead/max_depth/air_at_y/lava_ahead) via le wsite
// établi ne faisait RIEN (seul l'échec EAU armait le relocate) → _wsiteMineSet restait true → re-descente /home
// wsite au MÊME puits (galeries minées autour = drop_ahead partout) → boucle infinie. wsiteDrown ne couvre QUE
// le cas noyé. On COMPTE les échecs SECS via le wsite dans une fenêtre ; au seuil, le wsite mène à une impasse
...
const DEFAULT_THRESHOLD = 2;       // 2 échecs SECS via le wsite dans la fenêtre = impasse sèche (le 1er = blip toléré)

/**
 * Enregistre un échec de descente SEC via le wsite à l'instant `now` et décide s'il faut ABANDONNER le wsite.
 ...
 */
function recordWsiteStuck(times, now, opts = {}) {
...
module.exports = { recordWsiteStuck, DEFAULT_WINDOW_MS, DEFAULT_THRESHOLD };
