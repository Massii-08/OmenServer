...
const { recordWsiteStuck, DEFAULT_THRESHOLD, DEFAULT_WINDOW_MS } = require('./wsiteStuck');

test('1er échec SEC via le wsite → pas d\'abandon (blip toléré)', () => {
  const r = recordWsiteStuck([], 1000);
  assert.equal(r.abandon, false);
  assert.deepEqual(r.times, [1000]);
});

test('2e échec SEC dans la fenêtre → abandon du wsite (impasse sèche, live NethBot4 world_mn9)', () => {
  const r = recordWsiteStuck([1000], 5000);
...
test('échec hors fenêtre (>4 min) → compteur réinitialisé (pas d\'abandon)', () => {
  const r = recordWsiteStuck([1000], 1000 + 250000);  // >240 s plus tard
  assert.equal(r.abandon, false);
  assert.deepEqual(r.times, [1000 + 250000]);
});

test('seuil/fenêtre configurables', () => {
  const r = recordWsiteStuck([100, 200], 300, { threshold: 3, windowMs: 1000 });
  assert.equal(r.abandon, true);                       // 3 dans la fenêtre
  const r2 = recordWsiteStuck([100], 300, { threshold: 3, windowMs: 1000 });
  assert.equal(r2.abandon, false);                     // seulement 2
});

test('entrées non finies ignorées ; times absent → []', () => {
  const r = recordWsiteStuck(undefined, 500);
  assert.deepEqual(r.times, [500]);
  const r2 = recordWsiteStuck([NaN, null, 400], 500, { windowMs: 1000 });
