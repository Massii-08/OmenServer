...
test('IRON_ARMOR_CHAIN: pioche fer + kit bois sûr + four + food + 24 lingots → but = shield (1 lingot, avant l\'armure)', () => {
  ...
  const inv = {
    ...
    iron_ingot: 24, stone_sword: 1, cobblestone: 12, oak_planks: 24,
  };
...
test('IRON_ARMOR_CHAIN: kit prêt SANS nourriture → la descente n est PAS bloquée (chasse = hook best-effort)', () => {
  ...
  const inv = {
    ...
    stone_sword: 1, cobblestone: 12, oak_planks: 24,
  };
...
test('IRON_ARMOR_CHAIN: fer insuffisant, en surface → descend_y16 ; déjà à Y16 → iron_deep (branch-mine)', () => {
  const inv = {
    iron_pickaxe: 1, crafting_table: 1, stick: 4, furnace: 1, cooked_beef: 6,
    iron_ingot: 2, raw_iron: 1, stone_sword: 1, cobblestone: 12, oak_planks: 24,
  };
...
test('IRON_ARMOR_CHAIN: sous terre SANS food → food_stock ne stalle pas (met via y<45)', () => {
  const inv = {
    ...
    iron_ingot: 2, stone_sword: 1, cobblestone: 12, oak_planks: 24,
  };
...
test('DIAMOND_ARMOR_CHAIN: armure fer portée, 0💎, en surface → cobble buffer puis descente', () => {
  ...
  const g2 = firstUnmet(DIAMOND_ARMOR_CHAIN, ctx({ crafting_table: 1, iron_pickaxe: 1, stone_sword: 1, cobblestone: 20 }, full, 64));
...
test('DIAMOND_ARMOR_CHAIN: à -54 avec quelques 💎 mais pas assez → continue branch_mine (besoin RESTANT)', () => {
  const full = ['iron_helmet', 'iron_chestplate', 'iron_leggings', 'iron_boots'];
  const g = firstUnmet(DIAMOND_ARMOR_CHAIN, ctx({ crafting_table: 1, iron_pickaxe: 1, stone_sword: 1, cobblestone: 20, diamond: 10 }, full, -54));
...
test('firstUnmet: cobble+sticks+table, PAS de bois ni pioche → saute à stone_pickaxe (pas logs/wooden)', () => {
  const c = ctx({ stone_sword: 1, cobblestone: 5, stick: 4, crafting_table: 1 }, [], 16);
...
test('IRON_ARMOR_CHAIN: surface + kit + 1 seule pioche pierre → but = spare_picks (pré-stock 3)', () => {
  const inv = { stone_pickaxe: 1, crafting_table: 1, stick: 8, furnace: 1, stone_sword: 1, cobblestone: 12, oak_planks: 24 };
  const g = firstUnmet(IRON_ARMOR_CHAIN, ctx(inv, [], 64));
  assert.strictEqual(g.name, 'spare_picks');
});

test('IRON_ARMOR_CHAIN: surface + 3 pioches pierre → pré-stock satisfait, but = descend_y16', () => {
  const inv = { stone_pickaxe: 3, crafting_table: 1, stick: 8, furnace: 1, stone_sword: 1, cobblestone: 12, oak_planks: 24 };
  const g = firstUnmet(IRON_ARMOR_CHAIN, ctx(inv, [], 64));
  assert.strictEqual(g.name, 'descend_y16');
});

test('IRON_ARMOR_CHAIN: SOUS TERRE (y=16) + 1 pioche → PAS de pré-stock (continue de miner : iron_deep)', () => {
  const inv = { stone_pickaxe: 1, crafting_table: 1, stick: 6, furnace: 1, stone_sword: 1, cobblestone: 12, oak_planks: 24 };
  const g = firstUnmet(IRON_ARMOR_CHAIN, ctx(inv, [], 16));
...
test('IRON_ARMOR_CHAIN: sous terre 0 pioche + matériaux → re-craft en place (stone_pickaxe, a752743)', () => {
  const inv = { crafting_table: 1, stick: 6, furnace: 1, stone_sword: 1, cobblestone: 8, oak_planks: 24 };
  const g = firstUnmet(IRON_ARMOR_CHAIN, ctx(inv, [], 16));
  assert.strictEqual(g.name, 'stone_pickaxe');
});

test('IRON_ARMOR_CHAIN: pioche FER en poche → jamais de pré-stock pierre', () => {
  const inv = { iron_pickaxe: 1, crafting_table: 1, stick: 8, furnace: 1, stone_sword: 1, cobblestone: 12, oak_planks: 24 };
  const g = firstUnmet(IRON_ARMOR_CHAIN, ctx(inv, [], 64));
...
test('IRON_ARMOR_CHAIN: pioche fer + 24 raw_iron mais 0 combustible → but = armor_fuel', () => {
  const inv = { iron_pickaxe: 1, raw_iron: 24, stone_sword: 1, cobblestone: 12, crafting_table: 1, furnace: 1 };
  const g = firstUnmet(IRON_ARMOR_CHAIN, ctx(inv, [], 64));
...
test('IRON_ARMOR_CHAIN: fer brut + charbon + four → but = iron_armor (ensureArmor peut fondre+crafter)', () => {
  const inv = { iron_pickaxe: 1, raw_iron: 24, coal: 4, stone_sword: 1, cobblestone: 12, crafting_table: 1, furnace: 1 };
  const g = firstUnmet(IRON_ARMOR_CHAIN, ctx(inv, [], 64));
...
test('IRON_ARMOR_CHAIN: lingots DÉJÀ fondus (pas de smelt à faire) → pas d exigence combustible', () => {
  const inv = { iron_pickaxe: 1, iron_ingot: 24, stone_sword: 1, cobblestone: 12, crafting_table: 1, furnace: 1 };
  const g = firstUnmet(IRON_ARMOR_CHAIN, ctx(inv, [], 64));
  assert.strictEqual(g.name, 'iron_armor');
});
...
test('plank_buffer: kit descente prêt mais 0 bois en surface → firstUnmet = plank_buffer', () => {
  const inv = { stone_pickaxe: 3, stone_sword: 1, cobblestone: 12, crafting_table: 1, stick: 4, furnace: 1 };
...
test('plank_buffer: STOCK GÉNÉREUX (pattern AltoClef 16/07) — 24 équiv-planches (6 bûches) requis ; 8 ne suffisent plus', () => {
  const base = { stone_pickaxe: 3, stone_sword: 1, cobblestone: 12, crafting_table: 1, stick: 4, furnace: 1 };
...
test('plank_buffer: SOUS TERRE (y≤30) jamais bloquant (pas de remontée bois forcée)', () => {
  const inv = { stone_pickaxe: 1, stone_sword: 1, cobblestone: 12, crafting_table: 1, stick: 4, furnace: 1 };
