...
def build_coach_screen_prompt(context: Optional[Dict[str, Any]],
    ...
    return "\n\n".join([
        ...
        "heures où seule l'Europe est ouverte) ou ``tendance`` (un titre qui "
        "BOUGE aujourd'hui sur le marché US, connu ou non — c'est ta seule "
        "fenêtre sur ce qui n'est ni une grande valeur ni déjà dans ton "
        "radar). Ce n'est pas une note de qualité : un ``europe_pool`` sans "
        "rien de neuf ne mérite peut-être aucun dossier, un ``radar`` peut "
        "être daté — la source dit seulement d'où vient le titre, c'est TOI "
        "qui juges s'il mérite un dossier, et tu peux citer cette raison "
        "dans ta note. ⚠️ Un titre de tendance sans historique dans ta "
        "mémoire se joue à l'ENTRÉE de la fourchette de taille (10-15 %), "
        "jamais pleine taille — la tendance du jour est un projecteur, pas "
        "une thèse : il te faut quand même un niveau technique et une "
        "raison.",
