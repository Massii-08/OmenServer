...
(function () {
  ...
  /* data-dialog-name (ou data-name) des dialogues d'incitation à
     l'abonnement — noms de chunks webpack TradingView. */
  var UPSELL_NAME_RE =
    /gopro|go-pro|last-chance|offer|promo|upsell|thirty-day|early-bird|ad-free|adfree|no-?ads|paywall/i;
   ...
   * ``buttons`` : ``[{name, label, text}, ...]`` (chaînes, vides si absentes,
   * ``name``/``label`` = ``data-name``/``aria-label``, ``text`` = contenu).
   *
   * Priorité, la première trouvée gagne (recherchée sur TOUS les boutons
   * avant de passer au palier suivant) :
   *   1. ``name === 'close'`` ;
   *   2. libellé de fermeture porté par ``label`` (aria) ;
  ...
  function pickCloser(buttons) {
    ...
    for (i = 0; i < list.length; i += 1) {
      ...
      candidates.push({ index: i, name: String(raw.name || ''), label: label, text: text });
    }
    for (i = 0; i < candidates.length; i += 1) {
      if (normalizeText(candidates[i].name) === 'close') { return candidates[i].index; }
    }
    for (i = 0; i < candidates.length; i += 1) {
   ...
   * OU son ``text`` — jamais une alerte, une recherche, des paramètres, même
  ...
  function plan(snapshot) {
    ...
    for (i = 0; i < dialogs.length; i += 1) {
      ...
      if (dlgIndex !== -1) { actions.push({ kind: 'upsell', id: dialog.id, button: dlgIndex }); }
    }
    return actions;
  }
