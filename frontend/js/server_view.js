const ServerView = {
 ...
 return `
 <div style="display:flex;gap:8px;margin-bottom:12px;"><input id="sv-mods-q" class="form-input" placeholder="${Lang.t('sv.mod.search_mod_hint')}${verHint}" style="flex:1;" onkeydown="if(event.key==='Enter')ServerView._searchMods()" /><button class="btn btn-primary" onclick="ServerView._searchMods()">${Lang.t('sv.mod.search')}</button></div><div id="sv-mods-results"><div style="color:var(--text-muted)">${Lang.t('sv.mod.search_mod_desc')}</div></div>`;
 },
