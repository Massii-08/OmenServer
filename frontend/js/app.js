 ...
 */
 ...
 // Animation d'entrée de page (anim.js — no-op si prefers-reduced-motion)
 ...
 default:
 content.innerHTML = `
 <div class="text-center" style="padding: 60px; color: var(--text-muted);"><div style="font-size: 48px; margin-bottom: 16px;"></div><p>${Lang.t('module.unavailable')}</p><button class="btn btn-secondary mt-4" onclick="App.navigateTo('hub')">${Lang.t('access.back')}</button></div>
 `;
 }
 },
 ...
 */
 ...
 IDs preserved so monitoring.js continues to write here unchanged. --><div class="bento-overview" style="grid-template-columns:1.6fr 1fr 1fr 1fr;grid-template-rows:1fr;"><div class="stat-card main"><div class="label">CPU <span style="color:var(--text-dim);font-size:11px;">${t('dashboard.disk_combined')}</span><span class="blive"><span class="bdot"></span>LIVE</span></div><div class="value"><span id="stat-cpu-value">--</span><span class="unit">%</span></div><div id="stat-cpu-machines" class="stat-machines-list"></div><div class="stat-bar" id="stat-cpu-bar"></div><svg class="spark" id="hub-cpu-spark" viewBox="0 0 300 48" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="omen-agrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-opacity=".16"/><stop offset="100%" stop-opacity="0"/></linearGradient></defs><polygon class="area" points="" fill="url(#omen-agrad)"></polygon><polyline points=""></polyline><circle class="tip" r="2.5"></circle></svg></div><div class="stat-card"><div class="label">${t('dashboard.memory')} <span style="color:var(--text-dim);font-size:11px;">${t('dashboard.disk_combined')}</span></div><div class="value"><span id="stat-memory-value">--</span><span class="unit">%</span></div><div id="stat-memory-detail" class="footer">-- / -- Go</div><div id="stat-memory-machines" class="stat-machines-list"></div><div class="stat-bar" id="stat-memory-bar"></div></div><div class="stat-card"><div class="label">${t('dashboard.disk')} <span style="color:var(--text-dim);font-size:11px;">${t('dashboard.disk_combined')}</span></div><div class="value"><span id="stat-disk-value">--</span><span class="unit">%</span></div><div id="stat-disk-detail" class="footer">-- / -- Go</div><div id="stat-disk-machines" class="stat-machines-list"></div><div class="stat-bar" id="stat-disk-bar"></div></div><div class="stat-card"><div class="label">${t('dashboard.temp')} <span style="color:var(--text-dim);font-size:11px;">max</span></div><div class="value"><span id="stat-temp-value">--</span><span class="unit">°C</span></div><div id="stat-temp-machines" class="stat-machines-list"></div><div class="stat-bar" id="stat-temp-bar"></div></div></div><!-- Network of machines (PR24 — was modules-grid emoji cards before) --><div class="page-header" style="margin-top:24px;margin-bottom:12px;"><h2 style="font-size: 18px; font-weight: 700;">${t('nodes.title')} <span id="nodes-count" style="font-size:13px;font-weight:400;color:var(--text-dim);font-family:var(--font-mono);font-feature-settings:'tnum';"></span></h2></div><div id="nodes-grid" class="machines-grid"><div style="grid-column:1/-1;text-align:center;padding:24px;color:var(--text-dim);font-size:13px;">
 ${t('common.loading')}
 </div></div><!-- Game servers row-list (PR24 — Bento compact rows like MASTER §5) --><div class="page-header" style="margin-top:28px;margin-bottom:12px;"><h2 style="font-size: 18px; font-weight: 700;">${t('gs.title')}</h2></div><div id="hub-servers-list" class="row-list">${typeof Anim!=='undefined'?Anim.skeleton('row',3):`<div style="text-align:center;padding:16px;color:var(--text-dim);font-size:13px;">${t('common.loading')}</div>`}</div><!-- Planification globale --><div class="page-header" style="margin-top:28px;"><h2 style="font-size: 18px; font-weight: 700;">${t('scheduler.title')}</h2><p class="page-subtitle">${t('scheduler.subtitle')}</p></div><div id="hub-scheduler" style="background:var(--bg-elev-1);border-radius:var(--r-lg);padding:20px;border:1px solid var(--border);"><div style="text-align:center;padding:20px;color:var(--text-muted);font-size:13px;">${t('scheduler.loading')}</div></div>
 `;
 ...
 */
 ...
 if (isBot) {
 typeEl.innerHTML = `
 <option value="bot_start">${Lang.t('scheduler.bot_start')}</option><option value="bot_stop">${Lang.t('scheduler.bot_stop')}</option><option value="bot_restart">${Lang.t('scheduler.bot_restart')}</option>`;
 } else {
 typeEl.innerHTML = `
 <option value="backup">${Lang.t('scheduler.backup')}</option><option value="restart">${Lang.t('scheduler.restart')}</option>`;
 }
 },
 ...
 */
 ...
 const levelIcons = { ok: '', warning: '', critical: '' };
 ...
 */
 ...
 </div><div id="power-last-info" style="margin-top:12px;font-size:12px;color:var(--text-muted);"></div><div style="display:flex;gap:8px;margin-top:16px;align-items:center;"><button class="btn btn-primary" onclick="App._savePowerSchedule()">${t('power.save')}</button><button class="btn btn-secondary" onclick="App._testPower()" title="${t('power.test_desc')}">${t('power.test')}</button><button class="btn btn-secondary" onclick="App._cancelPowerWake()" style="font-size:12px;">${t('power.cancel')}</button><span id="power-msg" style="font-size:12px;margin-left:8px;"></span></div></div></div>
 ` : ''}
...
const SharingModal = {
 ...
 ${resourceType === 'bot' ? `
 <option value="start" selected>${Lang.t('sharing.bot_use')}</option><option value="manage">${Lang.t('sharing.bot_edit')}</option>
 ` : `
 <option value="view_only">${Lang.t('sharing.view_only')}</option><option value="start" selected>${Lang.t('sharing.start')}</option><option value="manage">${Lang.t('sharing.manage')}</option>
 `}
